...
class DiagramsComponent(project: Project,
  ...
  def setZoom(zoom: Zoom): Unit = {
    ...
    val preferredWidth = getPreferredSize.getWidth
    val viewRect = viewport.getViewRect
    val anchor = viewRect.getX + viewRect.getWidth
    val delta = anchor - viewRect.getX
    val ratio = anchor / preferredWidth

    currentZoom = zoom
    CompilationChartsToolWindowFactory.refresh(project)

    if (ratio.isFinite) invokeLater {
      val newPreferredWidth = getPreferredSize.getWidth
      val newViewPosition = new Point(
        (newPreferredWidth * ratio - delta).round.toInt,
        viewRect.y
      )
